...
def demo(args):
    ...
    with torch.no_grad():
        ...
        for (imfile1, imfile2) in tqdm(list(zip(left_images, right_images))):
            ...
            # disp0 = padder.unpad(agg[0])
            # disp1 = padder.unpad(agg[1])
            # disp2 = padder.unpad(agg[2])
            file_stem = imfile1.split('/')[-1]
            filename = os.path.join(output_directory,  f"{file_stem}.png")
            # filename0 = os.path.join(output_directory, 'agg0.png')
            # filename1 = os.path.join(output_directory, 'agg1.png')
            # filename2 = os.path.join(output_directory, 'agg2.png')
            
            disp = disp.cpu().numpy().squeeze()
            # disp0 = disp0.cpu().numpy().squeeze()
            # disp1 = disp1.cpu().numpy().squeeze()
            # disp2 = disp2.cpu().numpy().squeeze()
            plt.imsave(filename, disp.squeeze(), cmap='jet')
            # plt.imsave(filename0, disp0.squeeze(), cmap='jet')
            # plt.imsave(filename1, disp1.squeeze(), cmap='jet')
            # plt.imsave(filename2, disp2.squeeze(), cmap='jet')
