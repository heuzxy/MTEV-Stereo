...
def demo(args):
    ...
    with torch.no_grad():
        ...
        for (imfile1, imfile2) in tqdm(list(zip(left_images, right_images))):
            ...
            agg, disp = model(image1, image2, iters=args.valid_iters, test_mode=True)
            ...
            if args.save_png:
                disp_0 = np.round(disp0 * 256).astype(np.uint16)
                skimage.io.imsave(file_stem, disp_0)
                # disp_1 = np.round(disp1 * 256).astype(np.uint16)
                # skimage.io.imsave(file_stem, disp_1)
                # disp_2 = np.round(disp2 * 256).astype(np.uint16)
                # skimage.io.imsave(file_stem, disp_2)
