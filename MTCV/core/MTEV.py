...
class MTEVStereo(nn.Module):
    def __init__(self, args):
        ...
        self.region_division = RegionDivisionModule()
        self.context_enhance = GlobalContextEnhancement(in_channels=32)
        ...
        self.patch0 = nn.Conv3d(8, 8, kernel_size=(4, 1, 1), stride=(4, 1, 1), bias=False)
        self.patch1 = nn.Conv3d(8, 8, kernel_size=(8, 1, 1), stride=(8, 1, 1), bias=False)
        ...
        self.uncertainty_fusion = UncertaintyGuidedFusion(in_channels=96)
        # self.xcit = XCiT(embed_dim=128, depth=1, num_heads=8, mlp_ratio=1, separate=False)
        # self.sobelx = MySobelx()
        # self.one2three=nn.Sequential(
        #     nn.Conv2d(1,3,1,1,bias=False),
        #     nn.InstanceNorm2d(3),
        #     nn.LeakyReLU()
        # )
    def freeze_bn(self):
    ...
    def forward(self, image1, image2, iters=12, test_mode=False):
        ...
        with autocast(enabled=self.args.mixed_precision):
            ...
            # sobel_left=self.sobelx(image1)
            # sobel_right=self.sobelx(image2)
            # sobel_left= self.one2three(sobel_left)
            # sobel_right=self.one2three(sobel_right)
            # sobel_left = self.feature(sobel_left)
            # sobel_right = self.feature(sobel_right)
            attention_map = self.region_division(features_left)  # [B, 1, H, W] 高频区域
            features_left[0]=attention_map*features_left[0]
            features_right[0]=(1-attention_map)*features_right[0]
            ...
            # sobel_volume=build_gwc_volume(sobel_left[0], sobel_right[0], self.args.max_disp//4, 8)


            # disp_volume0 = all_disp_volume
            # disp_volume1 = self.patch0(all_disp_volume)
            # disp_volume2 = self.patch1(all_disp_volume)

            geo_encoding_volume0 = self.cost_agg0(all_disp_volume, features_left)
            # geo_encoding_volume1 = self.cost_agg1(disp_volume0, features_left)
            # geo_encoding_volume2 = self.cost_agg2(disp_volume0, features_left)
            geo_encoding_volume1 = self.patch0(geo_encoding_volume0)
            geo_encoding_volume2 = self.patch1(geo_encoding_volume0)

            cost_volume0 = self.classifier(geo_encoding_volume0)
            prob_volume0 = F.softmax(cost_volume0.squeeze(1), dim=1)
            agg_disp0 = disparity_regression(prob_volume0, self.args.max_disp//4)

            cost_volume1 = self.classifier(geo_encoding_volume1)
            prob_volume1 = F.softmax(cost_volume1.squeeze(1), dim=1)
            agg_disp1 = disparity_regression(prob_volume1, self.args.max_disp//16)
        
            cost_volume2 = self.classifier(geo_encoding_volume2)
            prob_volume2 = F.softmax(cost_volume2.squeeze(1), dim=1)
            agg_disp2 = disparity_regression(prob_volume2, self.args.max_disp//32)

            uncertainty_detail = disparity_variance(prob_volume_detail, self.args.s_disp_range,
                                                    disp_detail.unsqueeze(1))
            uncertainty_weak = disparity_variance(prob_volume_weak, self.args.m_disp_range // 2,
                                                  (disp_weak / 2).unsqueeze(1))
            uncertainty_textureless = disparity_variance(prob_volume_textureless, self.args.max_disp // 16,
                                                         (disp_textureless / 4).unsqueeze(1))

            # 10. 不确定性引导的特征融合
            fusion_weights = self.uncertainty_fusion(
                agg_disp0, agg_disp1, agg_disp2,
                uncertainty_detail, uncertainty_weak, uncertainty_textureless,
                features_left[0]
            )



            cnet_list = self.context_enhance(image1)
            net_list = [torch.tanh(x[0]) for x in cnet_list]
            inp_list = [torch.relu(x[1]) for x in cnet_list]
            inp_list = [list(conv(i).split(split_size=conv.out_channels//3, dim=1)) for i,conv in zip(inp_list, self.context_zqr_convs)]
            # global_context = self.xcit(inp_list[0][0])
            
        geo_block = Combined_Geo_Encoding_Volume
        # geo_block_s = Combined_Geo_Encoding_Volume_sobel
        ...
        # geo_fn_s=geo_block_s(sobel_volume.float(),radius=self.args.corr_radius,num_levels=self.args.corr_levels)
        b, c, h, w = match_left.shape
        coords = torch.arange(w).float().to(match_left.device).reshape(1,1,w,1).repeat(b, h, 1, 1)
        disp = agg_disp0+agg_disp1+agg_disp2
        iter_preds = []
        ...
        for itr in range(iters):
            ...
            # geo_feats=geo_fn_s(disp, coords)
            with autocast(enabled=self.args.mixed_precision):
                net_list, mask_feat_4, delta_disp = self.update_block(net_list, inp_list, geo_feat0, geo_feat1, geo_feat2, init_corr, fusion_weights, disp, global_context,iter16=self.args.n_gru_layers==3, iter08=self.args.n_gru_layers>=2)
