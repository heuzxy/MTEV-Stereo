...
class BasicDispEncoder(nn.Module):
    def __init__(self, args):
        ...
        # self.convc1 = nn.Conv2d(96*3+18, 128, 1, padding=0)
...
class BasicMultiUpdateBlock(nn.Module):
    def __init__(self, args, hidden_dims=[]):
        ...
        # self.geo_encoders = GeoEncoder(geo_planes=2 * (2*args.corr_radius + 1) * 8)
        ...
        self.aggregator =XCiT(embed_dim=128, depth=1, num_heads=8, mlp_ratio=1, separate=True)
        self.mask_feat_4 = nn.Sequential(
            nn.Conv2d(hidden_dims[2], 64, 3, padding=1),
            nn.ReLU(inplace=True))

    def forward(self, net, inp, geo_feat0=None, geo_feat1=None, geo_feat2=None, init_corr=None, selective_weights=None, disp=None,global_context=None, iter04=True, iter08=True, iter16=True, update=True):
        ...
        if iter04:
            ...
            # geo_feats=self.geo_encoders(geo_feats)
            geo_feat = selective_weights[:,0:1]*geo_feat0 + selective_weights[:,1:2]*geo_feat1 + selective_weights[:,2:3]*geo_feat2
            # geo_feat = torch.cat([geo_feat0,geo_feat1,geo_feat2,init_corr], dim=1)
            geo_feat = torch.cat([geo_feat, init_corr], dim=1)
            disp_features = self.encoder(disp, geo_feat)
            motion_features_global = self.aggregator(global_context, disp_features)
            if self.args.n_gru_layers > 1:
                net[0] = self.gru04(net[0], *(inp[0]), motion_features_global, interp(net[1], net[0]))
            else:
