...
class usvlanddatasets(StereoDataset):
    def __init__(self, aug_params=None, root='/root/autodl-tmp/usvland/', dstype='Left_Img_Rectified'):
        super(usvlanddatasets, self).__init__(aug_params)
        self.root = root
        self.dstype = dstype
    ...
    def _add_things(self,):
        ...
        root = self.root
        left_images = sorted( glob(osp.join(root,  '*/*/Left_Img_Rectified/*.png')) )
        right_images = sorted( glob(osp.join(root,  '*/*/Right_Img_Rectified/*.png')) )
        disparity_images = [ im.replace(self.dstype, 'Disp_Map') for im in left_images ]
...
class SceneFlowDatasets(StereoDataset):
    def __init__(self, aug_params=None, root='/root/autodl-tmp/sceneflow/', dstype='frames_finalpass', things_test=False):
...
class ETH3D(StereoDataset):
    def __init__(self, aug_params=None, root='/root/autodl-tmp/ETH3D', split='training'):
...
class KITTI(StereoDataset):
    def __init__(self, aug_params=None, root='/root/autodl-tmp/KITTI/KITTI_2012/', image_set='training',thing_test=False):
        super(KITTI, self).__init__(aug_params, sparse=True, reader=frame_utils.readDispKITTI)
        assert os.path.exists(root)
        if thing_test==False:

            root_12 = '/root/autodl-tmp/KITTI/KITTI_2012/'
            ...
            root_15 = '/root/autodl-tmp/KITTI/KITTI_2015'
            ...
            root_12 = '/root/autodl-tmp/KITTI/KITTI_2012/'
            image1_list = sorted(glob(os.path.join(root_12, 'test', 'colored_0/*_10.png')))
            image2_list = sorted(glob(os.path.join(root_12, 'test', 'colored_1/*_10.png')))
            disp_list = sorted(
                glob(os.path.join(root_12, 'test', 'disp_occ_test/*_10.png')))

            root_15 = '/root/autodl-tmp/KITTI/KITTI_2015'
            image1_list += sorted(glob(os.path.join(root_15, 'training', 'image_0/*_10.png')))
            image2_list+= sorted(glob(os.path.join(root_15, 'training', 'image_1/*_10.png')))
            disp_list += sorted(
                glob(os.path.join(root_15, 'training', 'disp_occ_0_test/*_10.png'))) 
